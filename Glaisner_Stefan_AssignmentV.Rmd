...
The documentation website for the API reveals that there is a limit of 5,000 API calls per day and an additional rate limitation of five requests per second. Thus, I define an initial time limit of 0.21 seconds for API calls to not encounter any error messages on this behalf.
...
While intensively working with the API, I noticed that even though I specified an appropriate request limit per second, errors with the status code 429 ('Too Many Requests') were likely to occur. Accordingly, I implemented a safety check which repeats the operation for those pages where the GET request was unsuccessful, but with a longer rest between the requests.

``ticketmaster_call`` returns a list object containing the final data frame with the specified variables, a vector including the status codes of all GET requests (before and after redoing unsuccessful GET requests) and a list with some meta information such as the number of pages and cases. 
